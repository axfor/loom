...
> **实现状态：已全部建成**，三处与本文不同，都是有意的，见 [TODO.md](TODO.md)：
>
> 1. **下划线代空格保留**（§1「点号是原样匹配」不实现）—— 作者已定不废弃。
> 2. **裸字符串仍指「我们的同名节」**（§6.1 那张表的第二行不实现）—— 改成字面量会破坏真实树里 185 处写法，
>    这类改动该在版本门后做，不该藏在一个特性里。
> 3. **`Self:` 里的顶层名用 `##` 而不是 `#`** —— markdown 的 h1 是文档标题不是节，这是全局规则；
>    §0 和 §6.4 的例子写成 `# job`，那是本文写错了。
>
> 另外 §7.2 的 `split` 在本文里只有一个参数，但第二半需要一个名字，所以实现是
> `split(在哪, "第二半叫什么")`；`swap` 实现为**一次交换**而不是两次 `move`，因为两次 move 各自
> 对着未被对方改动的文档解析，合起来会丢掉一个。
...
**所以「一个产物一个文件」和「资源分散在多个文件」都成立**——而且**选哪个不影响代码怎么写**，因为三条路寻址方式完全相同。

判据：

| 内容 | 用哪个 | 因为 |
|---|---|---|
| 几行，和位置语句一起读才说得清 | `Self:` 内联 | 分开反而要跳两个文件 |
| 几十行以上 | `import` 外置 | 留在真实文件里才有 markdown 预览、lint、链接检查 |
| 我们的文件**就是**产物（`.sh` / `.js`） | 外部文件，必须 | 要能跑、能 lint、能被 `lm sync` 三方合并 |

换句话说：**合并是一个便利，不是一个目标。** 内容长到需要编辑器支持时，把它放回真实文件没有任何损失——寻址不变，只是来源换了一条路。
