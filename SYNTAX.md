...
>   实现选了保住后者，于是 `join` 和它实际等价的 `unwrap` 要求一致。
>
> 另外 §0 与 §6.4 的资源段例子原本写的是 `# job`。markdown 的 h1 是**文档的标题**，
> 不是其中一节——若把它当节点，每一节都成了它的子节点，「删掉标题」就等于「删掉整个文件」。
> 实测真实树里被织的 63 个 markdown **没有一个**只有 h1，所以那是规格的笔误，例子已改成 `## job`。
