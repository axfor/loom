...
### 6. 「一个产物一个文件」在长内容上是负收益

`getting-started` 合并后是「1 行 `align` + 300 行资源段」。两个文件时，内容那份是**真 markdown**：有预览、有 lint、有链接检查、能被 `lm sync` 三方合并。合并之后全没了。

文档写了「按产物逐个选」，但**没给判据**。真实数据里内容量的分布也没算过。
